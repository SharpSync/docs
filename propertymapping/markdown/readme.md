...
* [Main concepts for rules: Categories](#main-concept-for-rules)
* [List of available rules](#list-of-rules)
* [Example: Import rules](#example-import-rules)  
* [Example: Display rules](#example-display-rules)  
* [Example: Export rules](#example-export-rules)  

## Main concepts for Rules

### Concept: Rule categories
...
* User scriptable rules (Javascript) have access to 3 parameters, not just 1:
    
    |Parameter|Description|Note|
    |--|--|--|
    |s|The string value from the source|Can be modified by previous rule|
    |rowData|The entire row's data (all the cell values, modifications, differences, etc)|Cannot be modified, read-only|
    |pm|The property mapping object|Contains additional options such as "Is it read-only"|

  
### Concept: Order of rules
Each type of rule can be reordered within it's own category, but you cannot move an export rule before an import rule
...
<span style="color:orange;display:inline"> _NOTE:_</span> The rule running for NetSuite _is not aware of the data being imported from Onshape_. That means that when running a rule such as a javascript `Text Manipulation` rule, the rule runs only for one source at a time, never for both at the same time. 
...
For example:
Let's suppose that we have Datasource 1 (DS1) and Datasource 2 (DS2), the process would be:

#### Step: Setup the property mappings 

* Setup the 1st property column. Give it an accessor of `description` with a column mapping of property `description` for DS1 and for DS2
...
| --------------------- | -------------- | ---------------------------------------------------------------------------------------------------- |
| [Append Text](#append-text)          | Import / Export| Adds the specified text to the end of the cell value                                                  |
| [Calculate number](#calculate-number)       | Import / Export| Uses the cell value and performs a calculation. The result of the calculation replaces the cell value  |
| [Export manipulation](#export-manipulation)    | Export only | Runs the specified javascript expression when data is exported. Has the ability to remove rowData values                                       |
| [Format as decimal](#format-as-decimal-number)     | Import / Export| Converts the cell value to a number and adds the specified number of decimals. This does round the number |
| [Number between](#number-between)         | Display        | The number in the cell value must be between (inclusive) the numbers specified                        |
| [Prepend text](#prepend-text)           | Import / Export| Adds the specified text to the beginning of the cell value                                            |
...
| [Replace first instance](#replace-first-instance) | Import / Export| Replaces the first instance of the specified text with the new value                                  |
| [Round to nearest X](#round-to-nearest-x)     | Import / Export| Rounds the number to up or down the nearest specified digit                                           |
| [Select from Json](#select-from-json)       | Import / Export| Converts the cell value from text to a JSON object and returns the value given by the specified key    |
| [Set cell value](#set-cell-value)         | Import / Export| Sets the cell value to the specified text. Existing text is replaced                                  |
| [Set empty cells](#set-empty-cells)        | Import / Export| Set an empty (any cell that has only whitespace or no value) cell value to the specified text          |
| [Text contains](#text-must-contain-string)          | Display        | The cell value must contain the specified text                                                        |
| [Text ends with](#text-must-end-with-string)         | Display        | The cell value must end with the specified text                                                       |
| [Text evaluation](#text-evaluation)        | Display        | Evaluates the cell value given the JavaScript expression                                              |
| [Text is in list](#text-is-in-list)        | Display        | The cell value must be in the list of specified items (Comma separated)                               |
| [Text is not in list](#text-is-not-in-list)    | Display        | The cell value must not be in the list of specified items (Comma separated)                           |
| [Text is a number](#text-must-be-a-number)       | Display        | The cell value must not be numeric                                                                    |
| Text is exactly        | Display        | The cell value must exactly match the specified text                                                  |
| [Text is not a number](#text-must-not-be-a-number)   | Display        | The cell value must not be numeric                                                                    |
...
[Text must be in list](#text-must-be-in-list)  
[Text must not be in list](#text-must-not-be-in-list)  
[Text must be a number](#text-must-be-a-number)  
[Text must not be a number](#text-must-not-be-a-number)  
        </blockquote>
    </details>
...
#### Export manipulation
!["Export manipulation"](../images/exportManipulation.png )  
